...
*Example* Usage: `./[unattended_install.sh](Scripts/unattended_install.sh) /tmp/Pansift.app 2>&1 | tee pansift_install.log` 
 ...
 * `pansift_token.conf` contains a single string comprised of an 86 character hexadecimal string ending in a double equals "==" (so it's 88 characters long). This ZTP write token is available from the bucket details in your account. The write token for the bucket can be used by multiple agents hence a 'multiagent' bucket. [Contact support](https://pansift.com/contact) if you are using a "multiagent" bucket and want discrete tokens per agent rather than creating more buckets as a grouping boundary. This token only allows writes and not reads to a specific bucket.

> :warning: **This token only allows writes and not reads to a specific bucket.**
